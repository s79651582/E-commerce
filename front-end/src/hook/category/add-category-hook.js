...
export const AddCategoryHook = () => {
    ...
    const handleSubmit = async (event)=>{
        ...
        if(name === '' || selectedFile === null){
            ...
            setIsPress(true);
            // await dispatch(createCategory({ name: "fatema" , email "fatema@gmail.com"}));  ---->  لإرسال نص فقط 
            await dispatch(createCategory(formData));  // لارسال صور و نصوص
            setLoading(false);
        }
    ...
    useEffect( ()=>{
        if(loading === false){
            ...
            if(res.status === 201){
                ...
                notify('يجب ان يحتوي الاسم على 3 احرف على الاقل', 'error');
            }
            else{
